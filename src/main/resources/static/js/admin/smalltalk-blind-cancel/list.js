window.addEventListener("load", function () {
    ...
    let renderPagination = function (currentPage) {
        ...
        let totalPage = Math.ceil(_totalCount / 10); //총 페이지 수 = 글수/10
...
// =================mvc에서부터 페이지 그리기 위함==========
    async function fetchDataAndProcess() {
        try {
            const response = await fetch('/api/smalltalk-blind-cancels/count?q=');
            if (!response.ok) {
    ...
    let titleUrl = `/admin/smalltalk-blind-cancel/detail?id=`
    let query = "";
        ...
        for (let b of list) {
            ...
            let template = `
                        ...
                        <div class="title-box">
                            ...
                            <div class="category smalltalk" >스몰톡</div>
                        </div>
    ...
    inputQuery.addEventListener("keyup", async function (e) {
        ...
        if (e.key === "Enter") {
            ...
            let url = `/api/smalltalk-blind-cancels?q=${query}`;

            let response = await fetch(url)
            let text = await response.text();
            bind(text);
            let countList = await fetch(`/api/smalltalk-blind-cancels/count?q=${query}`);
            _totalCount = await countList.text();
            renderPagination(1);
    ...
    paging.onclick = async function (e) {
        ...
        let response = await fetch(`/api/smalltalk-blind-cancels?q=${query}&p=${selectedPage}`);
        let text = await response.text();
        bind(text);
        currentPg = selectPg;
