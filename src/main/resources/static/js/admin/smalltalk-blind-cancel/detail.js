window.addEventListener("load", function () {
    let smalltalkInfo = document.querySelector(".board-info")
    let btnDots = smalltalkInfo.querySelector(".dots-btn")
    let dotsList = smalltalkInfo.querySelector(".list")
    ...
    for (i = 0; i < statuses.length; i++) {
        statuses[i].onclick = async function (e) {
            ...
            let url = `/api/smalltalk-blind-cancels?id=` + id + `&s=` + statusNum;
