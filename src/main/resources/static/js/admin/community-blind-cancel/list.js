...
// =================mvc에서부터 페이지 그리기 위함==========
    async function fetchDataAndProcess() {
        try {
            const response = await fetch('/api/community-blind-cancels/count?q=');
            if (!response.ok) {
    ...
    let titleUrl = `/admin/community-blind-cancel/detail?id=`
    let query = "";
        ...
        for (let b of list) {
            ...
            let template = `
                        ...
                        <div class="title-box">
                            ...
                            <div class="category " >${b.categoryName}</div>
                        </div>
    ...
    inputQuery.addEventListener("keyup", async function (e) {
        ...
        if (e.key === "Enter") {
            ...
            let url = `/api/community-blind-cancels?q=${query}`;

            let response = await fetch(url)
            let text = await response.text();
            bind(text);
            let countList = await fetch(`/api/community-blind-cancels/count?q=${query}`);
    ...
    paging.onclick = async function (e) {
        ...
        let response = await fetch(`/api/community-blind-cancels?q=${query}&p=${selectedPage}`);
        let text = await response.text();
        bind(text);
        currentPg = selectPg;
