window.addEventListener("load", function () {
    let communityInfo = document.querySelector(".board-info")
    let btnDots = communityInfo.querySelector(".dots-btn")
    let dotsList = communityInfo.querySelector(".list")
    ...
    for (i = 0; i < statuses.length; i++) {
        statuses[i].onclick = async function (e) {
            ...
            let url = `/api/community-blind-cancels?id=` + id + `&s=` + statusNum;
