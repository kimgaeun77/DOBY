...
import {modal} from '/js/smalltalk/detail.js';
...
window.addEventListener("load", async function () {
    ...
    const smalltalkId = document.querySelector(".board-info .title-dots-btn-container .title").dataset.smalltalkId;
    ...
    async function saveSubComment(findTarget) {
        ...
        let response = await fetch(`/api/smalltalk-comments`, config);
        ...
        await getCommentCount(smalltalkId);
    }
    ...
    async function saveMainComment() {
        ...
        await getMainCommentList(smalltalkId);
        await getCommentCount(smalltalkId);
    }
    ...
    async function deleteCommentGood(mId, commentId, findTarget) {
        ...
        let response = await fetch(`/api/smalltalk-comment-goods/${mId},${commentId}`, config);
    ...
    async function addCommentGood(mId, commentId, findTarget) {
        ...
        let response = await fetch(`/api/smalltalk-comment-goods`, config);
        if (response.status !== 201)
            throw new Error("error occurred");
    ...
    async function deleteComment(target) {
        ...
        let response = await fetch(`/api/smalltalk-comments/${commentId}`, {
            method: "DELETE",
            headers: {
                "X-CSRF-Token": csrf
            }
    ...
    // 부모 댓글 목록 get
    async function getMainCommentList(smalltalkId) {

        let response = await fetch(`/api/smalltalk-comments?smalltalk-id=${smalltalkId}`);
    ...
    async function getSubCommentList(mainCommentId, subCommentList) {
        let response = await fetch(`/api/smalltalk-comments?smalltalk-id=${smalltalkId}&p-id=${mainCommentId}`);
        if (response.status === 200) {
            let list = await response.json();
    ...
    // 댓글 전체 수 get
    async function getCommentCount(smalltalkId) {
        let response = await fetch(`/api/smalltalk-comments/count?smalltalk-id=${smalltalkId}`);
