...
window.addEventListener("load", function () {
    ...
    async function save() {
        ...
        // 커뮤니티 저장
        let result = await saveCommunity(community);
        ...
        if (el.classList.contains("save-btn")) {
            ...
            if (result.isSuccess) {
                ...
                command.onclick = function (e) {
                    ...
                    if (target.classList.contains("ok-btn")) location.href = `/community/detail?id=${result.communityId}`;
                }
            ...
            command.onclick = async function (e) {
                let target = e.target;
                if (target.nodeName !== "BUTTON") return;
                if (target.classList.contains("ok-btn")) location.href = "/community/list";
    ...
    tags.onclick = function (e) {
        let el = e.target;
        if (el.nodeName !== "LI" || el.classList.contains("no-search-msg")) return;
