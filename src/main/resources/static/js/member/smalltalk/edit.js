...
window.addEventListener("load", function (){
    ...
    // 커뮤니티 태그 목록 전체 삭제
    async function deleteTag(smalltalkId){
        let config = {
            method: "DELETE"
        }
        let response = await fetch(`/api/smalltalk-tags/${smalltalkId}`, config);
    ...
    // 커뮤니티를 수정하는 함수
    async function editSmalltalk(smalltalk){
        let config = {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
    ...
    async function edit(){
        ...
        let smalltalkId = form.dataset.smalltalkId;
        ...
        // 커뮤니티 수정
        await editSmalltalk(smalltalk);
        
        // // 커뮤니티 태그 목록 전체 삭제
        await deleteTag(smalltalkId);
    ...
    saveCancelBtnBox.onclick = async function (e){
        ...
        if(el.classList.contains("save-btn")){
            ...
            if(result.isSuccess)
                location.href = `/smalltalk/detail?id=${result.smalltalkId}`;
            throw new Error("error occurred");
        }
    }
