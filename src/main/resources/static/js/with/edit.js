...
window.addEventListener('load', function () {
    ...
    let selectedItems = techSelectBox.querySelectorAll('.selected-box .list .item');
    ...
    getTechList().then(data => {
        ...
        for (const item of selectedItems)
            for (let i = 0; i < techData.length; i++)
                if (techData[i].id === parseInt(item.dataset.id))
                    techData[i].isSelect = true;
    ...
    let items = techPositionBox.querySelectorAll('.total-list>.item');
    ...
    minusButton.onclick = () => {
        ...
        itemCount--;
        techPositionList.removeChild(techPositionList.lastElementChild);
    };
    ...
    submitButton.onclick = async function () {
        ...
        if (isInputValid && isSelectValid && isSelectTech && isNotDuplicatePosition) {
            ...
            let response = await fetch(`/api/with-positions/${withId}`);
            ...
            for (let i = 0; i < capacityInputList.length; i++) {
                ...
                let positionData = {
                    ...
                    capacity: parseInt(capacityInputList[i].value),
                }
            ...
            for (const techId of techSelectList) {
                let data = {withId: parseInt(withId), techId: parseInt(techId)}
